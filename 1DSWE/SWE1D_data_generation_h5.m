...
for s = 1:length(sigma_vals)
    ...
    for m = 1:length(mu_vals)
        ...
        % Save data for the current sigma and mu value in HDF5 format
        filename = ['data_sigma_no', num2str(s), 'mu_no', num2str(m), '.h5'];
        fullpath = fullfile(folder, filename);
        
        % Create datasets in the HDF5 file
        h5create(fullpath, '/time_all', size(time_all));
        h5write(fullpath, '/time_all', time_all);

        h5create(fullpath, '/h0', 1);
        h5write(fullpath, '/h0', h0);

        h5create(fullpath, '/sigma', 1);
        h5write(fullpath, '/sigma', sigma);

        h5create(fullpath, '/mu', 1);
        h5write(fullpath, '/mu', mu);

        h5create(fullpath, '/x', size(x));
        h5write(fullpath, '/x', x);

        h5create(fullpath, '/h_all', size(h_all));
        h5write(fullpath, '/h_all', h_all);

        h5create(fullpath, '/u_all', size(u_all));
        h5write(fullpath, '/u_all', u_all);
    end
end
