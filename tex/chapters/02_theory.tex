...
Putting this together, we can rewrite~\eqref{eq:momentum_conservation} as
...
\subsection{The SWE in integral form}
Often it is more convenient to work with the integral form of the SWE, since the integral form of equations of the form~\eqref{eq:vector_form_1D} and~\eqref{eq:vector_form_1D_variables} allows discountinuous solutions.
An integral form of~\eqref{eq:vector_form_1D} is stated in~\cite{Toro2024}:
\begin{align}
    \frac{\text{d}}{\text{d}t} \int_{x_L}^{x_R} \mathbf{U}(x,t) \text{ d}x = \mathbf{F}(\mathbf{U}(x_L, t)) - \mathbf{F}(\mathbf{U}(x_R, t)) + \int_{x_L}^{x^R} \mathbf{S}(\mathbf{U})(x,t) \text{ d}x,
\end{align}
when integrated over the control volume $V$ in the $x-t$ plane
\begin{align*}
...
\begin{figure}[H]
    ...
    \caption{Illustration of a control volume $V$ in the $x-t$ plane. Illustration modified from~\cite{Toro2024}.} 
\end{figure}

The integral form of the SWE in 1D is given by
\begin{align}\label{eq:integral_form_1D_homogeneous}
    \frac{\text{d}}{\text{d}t} \int_{x_1}^{x_2} \mathbf{U}(x,t) \text{ d}x = \mathbf{F}(\mathbf{U}(x_1, t)) - \mathbf{F}(\mathbf{U}(x_2, t)),
\end{align}
...
First we integrate over $x$ from $x_L$ to $x_R$ to obtain
\begin{align}\label{eq:derive_integral_form_1D}
    \int_{x_1}^{x_2} \mathbf{U}_t \text{ d}x + \int_{x_1}^{x_2} \mathbf{F(U)}_x \text{ d}x = \int_{x_1}^{x_2} \mathbf{S(U)} \text{ d}x.
\end{align}
...
\begin{align}
    \int_{x_1}^{x_2} \mathbf{U}_t \text{ d}x = \mathbf{F}(\mathbf{U}(x_L, t)) - \mathbf{F}(\mathbf{U}(x_R, t)) + \int_{x_1}^{x_2} \mathbf{S(U)} \text{ d}x.
\end{align}
Then we integrate over time from $t_1$ to $t_2$ to get
\begin{align*}
    \int_{t_1}^{t_2} \int_{x_1}^{x_2} \mathbf{U}_t \text{ d}x \text{d}t = \int_{t_1}^{t_2} \mathbf{F}(\mathbf{U}(x_L, t)) \text{ d}t - \int_{t_1}^{t_2} \mathbf{F}(\mathbf{U}(x_R, t)) \text{ d}t + \int_{t_1}^{t_2} \int_{x_1}^{x_2} \mathbf{S(U)} \text{ d}x \text{d}t.
\end{align*}
