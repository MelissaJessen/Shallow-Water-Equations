...
We introduce the MUSCL (monotonic upstream-centered scheme for conservation laws) scheme for higher-order accuracy and discuss the FVM for the 1D linearized SWE in spherical coordinates.
...
Recall, that the linearized 1D LSWE in spherical coordinates are given by
\begin{subequations}\label{eq:linearized_LSWE_spherical}
    \begin{align}
...
The system of ODEs can then be solves using standard ODE solvers, such as the ERK4 method.
We discretize the spatial dimension $\theta$ into $N$ cells, and approximate the spatial derivatives using central differences:
\begin{subequations}\label{eq:discretized_LSWE_spherical}
    \begin{align}
    \frac{\partial u}{\partial \theta}|_{\theta = \theta_i} = \frac{u_{i+1} - u_{i-1}}{2 \Delta \theta}, \\
    \frac{\partial h'}{\partial \theta}|_{\theta = \theta_i} = \frac{h'_{i+1} - h'_{i-1}}{2 \Delta \theta},
    \end{align}
\end{subequations}
for $i = 0,1, \dots, N-1$.
...
\begin{subequations}\label{eq:ODE_LSWE_spherical}
    \begin{align}
...
The inital conditions for $y$ are a Gaussian perturbation in water height and zero velocity:
\begin{align}
    y_0 = \begin{bmatrix}
