...
The Finite Volume Formula is based on the integral form~\eqref{eq:integral_form_1D_final}, which we divide by the length of the cell $\Delta x$ to get
\begin{align*}
...
\end{align*}
For a finite volume $V_i^n$ we average the terms over the volume to get the explicit conservative formula:
...
\end{align}
The value $\mathbf{U}_i^n$ is defined as the average value over the $i$-th cell at time $t_n$:
\begin{align}
    \mathbf{U}_i^n = \frac{1}{\Delta x} \int_{x_{i-1/2}}^{x_{i+1/2}} \mathbf{U}(x,t_n) \text{ d}x,
\end{align}
and is also referred to as the cell average.
The value $\mathbf{F}_{i-1/2}^n$ is the average flux along the line $x = x_{i-1/2}$ at time $t_n$:
\begin{align*}
    \mathbf{F}_{i-1/2}^n = \frac{1}{\Delta t} \int_{t_n}^{t_{n+1}} \mathbf{F}(\mathbf{U}(x_{i-1/2},t)) \text{ d}t,
\end{align*}
and the source term $\mathbf{S}_i$ is the average source term over the $i$-th cell at time $t_n$:
\begin{align*}
