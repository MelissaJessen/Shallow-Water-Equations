...
In \autoref{sec:numerical_results} we present the results from the finite volume method (FVM), where we have implemented the method for solving the shallow water equations in 1D  and 2D and tested it on several problems.
We also address the scalability issues of the finite volume method.
In \autoref{sec:data-driven-results} we present the results from the data-driven methods, where we have implemented neural networks and Fourier neural operators to solve the shallow water equations in 1D.
...
Finally, we have tested the implementation on the 2D idealised circular dam break problem, which is also from Toro's book~\cite{Toro2024}.
