function ulimit = SlopeLimitN(x,u,param)
...
% Computes average by removing all must constant modal coefficients and
% converting back to nodal form.
...
if(~isempty(ids))
  ...
  ulimit(:,ids) = SlopeLimitLin(ul,x(:,ids),vkm1(ids),vk(ids),vkp1(ids),param);
end
return;
