...
% Numerical Methods for Conservation Laws Hesthaven
...
%van Albada limiter
if(type==4)psi=minmod([(((a.^2+c.^2).*b+(b.^2+c.^2).*a)./(a.^2+b.^2+c.^2)); 2*a2*b]);end
%van Leer limiter
if(type==5)psi=minmod([2.*a.*b./(a+b); 2*a; 2*b]);end
%TVB limiting
if(type==6)psi=minmodTVB([(a+b)/2;2*a;2*b],M,h);end
%Woordward
if(type==7)psi=minmod([abs(a+b)/2; 2*a; 2*b]);end
% if(type==8)psi = SlopeLimit1(x,u,param);end
